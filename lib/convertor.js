...
class MongoToKnex {
    ...
    groupRelationStatements(statements, mode) {
        ...
        _.each(innerWhereStatements, (statement) => {
            ...
            if (!createSubGroup && group[statement.config.tableName]) {
                createSubGroup = _.find(group[statement.config.tableName].innerWhereStatements, (innerStatement) => {
                ...
                if (_.every(statements.map(s => s.operator), isCompOp)) {
                    ...
                    qb[reference.whereType](`${this.tableName}.id`, comp, function () {
                        ...
                        _.each(statements, (statement, key) => {
                            ...
                            const statementColumn = `${statement.join_table || statement.config.tableName}.${statement.column}`;
                            let statementOp;
